...
class Cp extends My_Controller {
	...
	private function _init()
	{
		...
		$this->set_panel('aries_admin/users');
	...
	public function Index($page = 'index'){
		if (!file_exists('application/views/aries_admin/users/'.$page.'.php'))
		{
			redirect('admins/error', 'refresh');		
		}
		
		$this->set_activepage($page);
		$this->data['title'] = "TechFocus Ltd - Focusing on Technology" ;
		$this->data['activepage'] = "User Admin" ;
		...
		else
		{
			...
			$this->load->view($this->template_dir.$page, $this->data);
	...
	function login($page = 'login'){
		...
		if ($this->form_validation->run() == true)
		{
			...
			else
			{
				...
				redirect('admin/cp/login', 'refresh'); // use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		}
		else
		{
	...
	function change_password($page="change_password")
	{
		if (!file_exists('application/views/aries_admin/users/'.$page.'.php'))
		{
		//Sorry
		...
		if ($this->form_validation->run() == false)
		{
			...
			// render
			$this->_render_page($this->template_dir.$page, $this->data);
		}
		else
		{
			...
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect('admin/cp/change_password', 'refresh');
			}
		}
	}
	...
	function forgot_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$this->_render_page($this->template_dir. 'forgot_password', $this->data);
		}
		else
		{
			...
			if(empty($identity)) {
		                ...
		                $this->session->set_flashdata('message', $this->ion_auth->messages());
                		redirect("admin/cp/users/forgot_password", 'refresh');
            		}
			...
			if ($forgotten)
			{
				...
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				redirect("admin/cp/users/login", 'refresh'); //we should display a confirmation page here instead of the login page
			}
			else
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user)
		{
			...
			if ($this->form_validation->run() == false)
			{
				...
				// render
				$this->_render_page($this->template_dir. 'reset_password', $this->data);
			}
			else
			{
				...
				else
				{
					...
					if ($change)
					{
						// if the password was successfully changed
						$this->session->set_flashdata('message', $this->ion_auth->messages());
						redirect("admin/cp/users/login", 'refresh');
					}
					else
					{
						$this->session->set_flashdata('message', $this->ion_auth->errors());
						redirect('admin/cp/users/reset_password/' . $code, 'refresh');
					}
				}
			}
		}
	...
	function activate($id, $code=false)
	{
		...
		else
		{
			...
			$this->session->set_flashdata('message', $this->ion_auth->errors());
			redirect("users/admin/forgot_password", 'refresh');
		}
	}
	...
	function deactivate($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->_render_page($this->template_dir.'deactivate_user', $this->data);
		}
		else
		{
	...
	function delete_user($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->_render_page('aries_admin/users/delete_user', $this->data);
		}
		else
		{
			// do we really want to deactivate?
	...
	function edit_user($id)
	{
		...
		$this->_render_page('aries_admin/users/edit_user', $this->data);
	}
	...
	function create_group()
	{
		...
		else
		{
			...
			$this->_render_page('aries_admin/users/create_group', $this->data);
		}
	}
	...
	function edit_group($id)
	{
		...
		$this->_render_page('aries_admin/users/edit_group', $this->data);
	}
